...
Answer:
Form validation is the process of ensuring that user input is clean, correct, and useful. For example if user register to become a member of a website,
in the registration form user is required to input his email, the email should be valid and unique so the form validation is process to check if user input an email following the requirement.
